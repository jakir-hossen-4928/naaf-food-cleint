...
import { Search, Edit, Trash2, Plus, Truck, Filter, Download, RefreshCw, Eye, MoreHorizontal } from "lucide-react"
...
import { formatCurrency, formatPhoneNumber, OrderFormData } from "@/lib/validation"
...
export function Orders() {
  ...
  const { orders, isLoading, createOrder, updateOrder, deleteOrder, dispatchOrder, isCreating, isUpdating, isDeleting, isDispatching } = useOrders()
  ...
  const handleCreateOrder = useCallback((data: OrderFormData) => {
    createOrder({
      ...data,
  ...
  const handleUpdateOrder = useCallback((data: OrderFormData) => {
    if (editingOrder) {
  ...
  const handleSendToSteadfast = useCallback(async (orderId: string, orderNumber: string) => {
    try {
      await dispatchOrder(orderId)
      toast({
        title: "Success",
        description: `Order ${orderNumber} dispatched to Steadfast successfully`,
      })
    } catch (error) {
      toast({
        title: "Error",
            ...
            <div className="flex justify-between items-start">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {user?.role === 'Admin' && !order.steadfast_tracking_id && order.status !== 'Cancelled' && (
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {formatPhoneNumber(order.mobile_number)}
                            </p>
                            {order.email && (
